...
def ask_scenario_settings(candidate_count: int):
    ...
    if scenario == "mixed_base_sequences":
        ...
        print("Будут сгенерированы rectangle, triangle и hexagon в трёх рядах.")
        ...
        rectangles = take(
            per_type,
            gen_rectangle(width=1.2, height=0.8, gap=0.35, start=(0.0, rectangle_y)),
        )
        triangles = take(
            per_type,
            gen_triangle(side=1.1, gap=0.35, start=(0.0, triangle_y)),
        )
        hexagons = take(
            per_type,
            gen_hexagon(radius=0.5, gap=0.35, start=(0.0, hexagon_y)),
        )
    ...
    if scenario == "symmetric_triangles":
        ...
        top = take(
            ...
            map(
                tr_translate(0.0, y),
                gen_triangle(side=side, gap=gap),
            ),
        )
    ...
    if scenario == "zipped_triangles":
        ...
        upper = take(
            candidate_count,
            gen_triangle(side=side, gap=gap, start=(0.0, y)),
        )
...
def make_filters() -> tuple:
    ...
    if ask_yes_no("Фильтровать по кратчайшей стороне?", default=False):
        max_side = ask_float("Оставить полигоны с кратчайшей стороной меньше", 10.0)
    ...
    if ask_yes_no("Фильтровать по вершинам другого полигона внутри?", default=False):
        print("Будет создан контрольный rectangle. Останутся полигоны, содержащие хотя бы одну его вершину.")
    ...
    if ask_yes_no("Фильтровать пересекающиеся с контрольным rectangle?", default=False):
        print("Будет создан контрольный rectangle. Останутся полигоны, которые с ним пересекаются.")
...
def print_result(polygons):
    ...
    if not polygons:
        print("Полигоны не найдены. Попробуйте ослабить фильтры или увеличить количество кандидатов.")
        return
